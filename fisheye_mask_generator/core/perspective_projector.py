...
class PerspectiveProjector:
    ...
    def __init__(
        ...
        if pitch_steps == 1:
            ...
            # Dynamic pitch calculation for 25% overlap based on FOV
            overlap_percentage = 0.25
            effective_coverage = fov * (1.0 - overlap_percentage)
            pitch_separation = effective_coverage
            
            # Create two pitch angles symmetric around horizon (0°)
            self.pitch_angles = [-pitch_separation / 2, pitch_separation / 2]
        else:
            # For more than 2 pitch levels, use the original linear spacing
